...
TF1* FitTools::fitSingleElectronPeak1Chan( const std::string& outputdir, const std::string& name, TTree* tree, int niter, float nSigma, int chanNr ) {

  std::string histoName(Form("h1_%s", name.c_str()));
  TH1D* h1 = new TH1D(histoName.c_str(), "", 200, 0., 6000.);
  tree->Project( histoName.c_str(),Form( "cef3_corr[%d]", chanNr), "(isSingleEle_scintFront && nHodoClustersX==1 && nHodoClustersY==1)");
