...
int main( int argc, char* argv[] ) {
 ...
 TF1* energyfunc = fitSingleElectronPeakBGO( outputdir, runs[4], treeMean,iterations,fitSigmas );
  ...
  */
  ...
  float adcEnergyC = energyfuncC->GetParameter(1);
    ...
    TFile* energyfileS = TFile::Open(Form("OriginalSimulationData/RealWHodo/Reco_%s.root", simulation[4].c_str()));
  // TFile* energyfileS = TFile::Open(Form("OriginalSimulationData/newReal/Reco_%s.root", simulation[4].c_str()));
    ...
    //ResoStruct energyrsS = getResponseResolutionMC( outputdir, energytreeS, simulationIdeal[4], beamEnergySimulation[4] );
...
for( unsigned i=0; i<runs.size(); ++i ) {
    ...
    //////BGO /////////////
    TF1* funcBGO = fitSingleElectronPeakBGO(outputdir, runs[i],tree,iterations , fitSigmas);
...
//For the simulated BGOs
    TFile* fileS = TFile::Open(Form("analysisTrees_%s/Reco_%s.root", tag.c_str(), simulation[i].c_str()));
    ...
    float meanSerr = thisFuncS->GetParError(1);    std::cout << "meanS bgo = " << meanS << std::endl;    std::cout << "mean Energy ADC = " << adcEnergy << std::endl;    std::cout << adcEnergy / meanS << std::endl;    meanS = meanS /52.1293 * adcEnergy;
    gr_resp_vs_energy_bgo_simul->SetPoint(i, energyS, meanS);    gr_resp_vs_energy_bgo_simul->SetPointError(i, energyErrS, meanSerr);
}
  ...
  */
  ...
  for( unsigned i=0; i<runs.size(); ++i ) {
    ...
    //TFile* fileS = TFile::Open(Form("OriginalSimulationData/RealScint/Reco_%s.root", simulation[i].c_str()));
    TFile* fileS = TFile::Open(Form("OriginalSimulationData/RealWHodo/Reco_%s.root", simulation[i].c_str()));
    //TFile* fileS = TFile::Open(Form("OriginalSimulationData/RealEnergy/Reco_%s.root", simulation[i].c_str()));
  ...
  gr_sigmaSimulPS->SetPoint(i, beamEnergySimulation[i], resoPS );
    ...
    ///////////////// 5x5 matrix //////////////////////////////
   TFile* fileF= TFile::Open(Form("OriginalSimulationData/mat5/Reco_%s.root",  simulationIdeal[i].c_str()));
...
gr_respIdeal->SetPoint( i, beamEnergySimulation[i], rsF.resp*adcEnergyC/energy491S);
gr_respIdeal->SetPointError( i,0,  0 );
   ...
   TFile* fileA= TFile::Open(Form("OriginalSimulationData/ActFirstIdeal/Reco_%s.root",  simulationIdeal[i].c_str()));
    ...
    gr_flipped5->SetPoint( i, beamEnergySimulation[i], rs_psA.resp*adcEnergyC/energy491S );
    gr_flipped5->SetPointError( i,0, 0. );
 ...
 */
  ...
  leg0->AddEntry( (TObject*)0, Form("chi2/ndf=%.0f\n / %d",f1_lines->GetChisquare(), f1_lines->GetNDF() ), "");
  leg0->SetFillColor(0);
  leg0->Draw("same");
  ...
  /////////// REVERSED SETUP LINEARITYYYYYYYYY////////////////
  TH2D* h2_axes5 = new TH2D( "axes", "", 10, 0., xMax, 10, 0., 4500. );
  h2_axes5->SetXTitle("Electron Beam Energy [MeV]");
  h2_axes5->SetYTitle("CeF3 Response [ADC Counts]");
  h2_axes5->Draw("");

  gr_resp_vs_energy_simul->SetMarkerStyle(20);
  gr_resp_vs_energy_simul->SetMarkerSize(1.6);
  gr_resp_vs_energy_simul->SetMarkerColor(46);
  gr_resp_vs_energy_simul->Draw("p same");
...
gr_flipped5->Draw("p same");
  ...
  gr_resp_vs_energy_simul->Fit(f1_lines2,"RN");
  ...
  leg9->AddEntry(gr_resp_vs_energy_simul,"MC (1x1)","p");
  leg9->AddEntry(f1_lines2,Form("offset=%.0f\n #pm %.0f\n",f1_lines2->GetParameter(0), f1_lines2->GetParError(0) ),"L");
  leg9->AddEntry( (TObject*)0, Form("chi2/ndf=%.0f\n / %d",f1_lines->GetChisquare(), f1_lines2->GetNDF() ), "");
...
//leg4->AddEntry( (TObject*)0, Form("chi2/ndf=%.2f\n / %d",fun->GetChisquare(), fun->GetNDF() ), "");
  ...
  TF1 *fun2= new TF1("fun2", "sqrt([0]*[0]/(x))",50, xMax);
...
leg->AddEntry(gr_sigmaSimulPS,"MC +PS","p");
...
void doSingleFit( TH1D* h1, TF1* f1, const std::string& outputdir, const std::string& name, int niter, float nSigma ) {
  ...
  //c1->cd();
  // h1->Draw();
  //c1->SaveAs( Form("%s/fit_gaus09_%s.pdf", outputdir.c_str(), name.c_str()) );
  //delete c1;
}
...
ResoStruct getResponseResolutionMC( const std::string& outputdir, TTree* tree, const std::string& name, float energyS, bool withHodo ) {
  ...
  TF1* f1 = new TF1( Form("f1_%s", name.c_str()), "gaus", 0., 500.);
...
TH1D* projectHodoMC ( TTree* tree, const std::string& name , bool withHodo){
  ...
  TH1D* h1 = new TH1D(name.c_str(), "", 200, 0., 500.);
